...
\begin{itemize}
    \item \textbf{Frontend:} ReactJS (ViteJS) – do phổ biến, cộng đồng lớn, dễ mở rộng và phù hợp xây dựng giao diện phức tạp.
...
\section{Use Case}
\subsection{Sơ Đồ Use Case Cho Hệ Thống}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../usecases/system/system.drawio.png}
    \caption{Sơ đồ Use Case cho hệ thống}
    % \label{fig:usecase}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../usecases/detailed/account_management.drawio.png}
    \caption{Sơ đồ Use Case cho Account Management}
    % \label{fig:usecase}
\end{figure}
...
\hline
\vspace{0.025cm}
\textbf{Use Case} & \vspace{0.025cm} Manage Balance \\
\textbf{Actor} & Customer, Credit Payment \\
\textbf{Mục tiêu} & Quản lý số dư trong ví bằng cách chuyển đổi giữa tiền - điểm, hoặc xem lịch sử giao dịch. \\
\textbf{Precondition} & 
    Người dùng đã đăng nhập vào hệ thống.

    Người dùng có ví điện tử hoặc tài khoản điểm liên kết. \\
\textbf{Postcondition} & 
    Số dư trong ví/điểm được cập nhật chính xác.

    Giao dịch được lưu vào \textit{Transaction History}. \\
\hline
\vspace{0.025cm}
\textbf{Main flow} & 
\begin{enumerate}
    \item Người dùng chọn chức năng \textit{Manage Balance}.
    \item Hệ thống hiển thị các tùy chọn:
    \begin{itemize}
        \item Money to Points
        \item Points to Money
        \item View Transaction History
    \end{itemize}
    \item Nếu người dùng chọn \textit{Money to Points}:
    \begin{itemize}
        \item[3.1.] Hệ thống yêu cầu chọn số tiền và phương thức thanh toán (\textit{Select Payment Method}).
        \item[3.2.] Hệ thống xử lý giao dịch và cập nhật ví (\textit{Update Wallet}).
        \item[3.3.] Hệ thống thông báo kết quả thành công.
    \end{itemize}
    \item Nếu người dùng chọn \textit{Points to Money}:
    \begin{itemize}
        \item[4.1.] Hệ thống yêu cầu nhập số điểm cần đổi và chọn phương thức nhận tiền (use case: \textit{Select Payment Method}).
        \item[4.2.] Hệ thống xử lý giao dịch và cập nhật ví (\textit{Update Wallet}).
        \item[4.3.] Hệ thống thông báo kết quả.
    \end{itemize}
    \item Nếu người dùng chọn \textit{View Transaction History}:
    \begin{itemize}
        \item[5.1.] Hệ thống hiển thị danh sách các giao dịch trước đó.
    \end{itemize}
\end{enumerate} \\
\hline
\vspace{0.025cm}
...
\begin{itemize}
    \item[3.1a.] Nếu số dư không đủ để quy đổi $\rightarrow$ hiển thị thông báo lỗi.
    \item[3.2a.] Nếu giao dịch bị gián đoạn (mạng lỗi, thanh toán thất bại) $\rightarrow$ hiển thị thông báo thất bại.
    \item[4.1a.] Nếu người dùng nhập số điểm vượt quá số điểm hiện có $\rightarrow$ hiển thị lỗi.
\end{itemize} \\
\hline
\end{tabular}

\subsubsection{Manage Balance}
\subsubsection*{* Diagram}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../usecases/detailed/manage_balance.drawio.png}
    \caption{Sơ đồ Use Case cho Manage Balance}
    % \label{fig:usecase}
\end{figure}
...
\hline
\end{tabular}
\caption{Use Case Scenario: Join event}
\end{table}
\subsubsection{Join Event}
\subsubsection*{* Diagram}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../usecases/detailed/join_event.drawio.png}
    \caption{Sơ đồ Use Case cho Support}
    % \label{fig:usecase}
\end{figure}
\newpage
\begin{table}[h!]
...
\hline
\vspace{0.025cm}
\textbf{Use Case}& \vspace{0.025cm}  Join Event\\ 
\textbf{Actor} & Customer, Admin \\ 
\textbf{Mục tiêu} & Khách hàng có thể tham gia sự kiện, xem thể lệ, nhận thông báo và nhận quà khi đủ điều kiện. Ngoài ra, khách hàng có thể tạo sự kiện nhưng cần được quản trị viên phê duyệt. Quản trị viên có toàn quyền tạo, chỉnh sửa hoặc xóa sự kiện. \\ 
\textbf{Precondition} & 
    Actor đã đăng nhập vào hệ thống.  

    Trong hệ thống đã có các sự kiện sẵn sàng. \\ 
\textbf{Postcondition} & 
	
    Khách hàng tham gia sự kiện, nhận thông báo hoặc nhận quà.  

    Sự kiện do khách hàng tạo sẽ được lưu và đánh dấu “Chờ phê duyệt”.  

    Các thao tác quản trị của Admin được phản ánh trong hệ thống. \\ 
\hline
\vspace{0.025cm}
\textbf{Main flow} & 
\begin{enumerate}
    \item Tương tác của Khách hàng:
    \begin{itemize}
        \item Khách hàng tham gia sự kiện.
        \item Khách hàng xem thể lệ chương trình.
        \item Hệ thống gửi thông báo sự kiện cho khách hàng.
        \item Khách hàng hoàn thành điều kiện sự kiện.
        \item Khách hàng yêu cầu nhận quà.
        \item Hệ thống xác thực và cấp quà cho khách hàng.
    \end{itemize}
    \item Khách hàng tạo sự kiện:
    \begin{itemize}
        \item Khách hàng tạo và gửi sự kiện mới.
        \item Hệ thống lưu sự kiện và đánh dấu ``Chờ phê duyệt''.
        \item Quản trị viên được thông báo về sự kiện mới.
    \end{itemize}
    \item Tương tác của Quản trị viên:
    \begin{itemize}
        \item Quản trị viên duyệt sự kiện do khách hàng gửi.
        \item Quản trị viên có thể phê duyệt, chỉnh sửa hoặc từ chối sự kiện.
        \item Sự kiện được phê duyệt sẽ triển khai cho khách hàng.
        \item Quản trị viên cũng có thể tạo sự kiện mới hoặc cập nhật/xóa sự kiện hiện có.
    \end{itemize}
\end{enumerate} \\ 
\hline
\vspace{0.025cm}
\textbf{Alternative flow} & 
\begin{itemize}
    \item[1a.] Khách hàng yêu cầu nhận quà khi chưa đủ điều kiện $\rightarrow$ Hệ thống từ chối và thông báo lý do.
    \item[2a.] Quản trị viên từ chối sự kiện do khách hàng tạo $\rightarrow$ Hệ thống thông báo lý do từ chối cho khách hàng.
    \item[3a.] Quản trị viên chỉnh sửa hoặc xóa sự kiện đang diễn ra $\rightarrow$ Hệ thống gửi thông báo đến các khách hàng đã tham gia, khách hàng có thể bị hủy tham gia hoặc phải tham gia lại.
\end{itemize} \\ 
\hline
\end{tabular}
\caption{Use Case Scenario: Join event}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../usecases/detailed/support.drawio.png}
    \caption{Sơ đồ Use Case cho Support}
    % \label{fig:usecase}
\end{figure}
\newpage
...
\hline
\vspace{0.025cm}
\textbf{Use Case}& \vspace{0.025cm}  Support \\ 
\textbf{Actor} & Customer, Admin \\ 
\textbf{Mục tiêu} & Khách hàng có thể gửi yêu cầu hỗ trợ, theo dõi tiến trình xử lý, nhận phản hồi. Quản trị viên tiếp nhận, xử lý và quản lý các yêu cầu hỗ trợ để đảm bảo hiệu quả dịch vụ. \\ 
\textbf{Precondition} & 
    Khách hàng hoặc quản trị viên đã đăng nhập vào hệ thống.   

    Hệ thống đang trực tuyến và sẵn sàng xử lý yêu cầu. \\ 
\textbf{Postcondition} & 
    Yêu cầu hỗ trợ của khách hàng được ghi nhận và lưu trữ.  

    Quản trị viên đã xử lý hoặc phản hồi yêu cầu.  

    Lịch sử và phản hồi được lưu để tham chiếu sau này. \\ 
\hline
\vspace{0.025cm}
\textbf{Main flow} & 
\begin{enumerate}
    \item Tương tác của Khách hàng:
    \begin{itemize}
        \item Khách hàng truy cập mục hỗ trợ.
        \item Khách hàng tạo yêu cầu hỗ trợ mới kèm thông tin (đơn hàng,...).
        \item Hệ thống ghi nhận yêu cầu và cấp mã theo dõi.
        \item Khách hàng có thể theo dõi trạng thái và nhận thông báo cập nhật.
    \end{itemize}
    \item Tương tác của Quản trị viên:
    \begin{itemize}
        \item Quản trị viên xem danh sách yêu cầu hỗ trợ.
        \item Quản trị viên kiểm tra thông tin và xử lý vấn đề.
        \item Quản trị viên giải quyết hoặc phản hồi khách hàng.
        \item Quản trị viên cập nhật trạng thái và thông báo cho khách hàng.
        \item Lịch sử phản hồi và xử lý được lưu trữ.
    \end{itemize}
\end{enumerate} \\ 
\hline
\vspace{0.025cm}
\textbf{Alternative flow} & 
\begin{itemize}
    \item[1a.] Quản trị viên chưa thể xử lý ngay $\rightarrow$ yêu cầu thêm thông tin từ khách hàng, hệ thống gửi thông báo và chờ phản hồi.
    \item[2a.] Hệ thống bị gián đoạn $\rightarrow$ hiển thị thông báo lỗi và yêu cầu thử lại sau.
    \item[3a.] Khách hàng hủy yêu cầu hỗ trợ $\rightarrow$ Hệ thống cập nhật trạng thái là ``Đã hủy'' và thông báo cho quản trị viên.
\end{itemize} \\ 
\hline
\end{tabular}
\caption{Use Case Scenario: Support}
\end{table}
\newpage
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../usecases/detailed/moderation.drawio.png}
    \caption{Sơ đồ Use Case cho Moderation}
    % \label{fig:usecase}
\end{figure}
...
\hline
\vspace{0.025cm}
\textbf{Use Case} & \vspace{0.025cm} Moderation \\
\textbf{Actor} & Admin, Customer \\
\textbf{Mục tiêu} & Quản lý và đảm bảo hệ thống vận hành minh bạch, an toàn thông qua việc kiểm duyệt sản phẩm, tài khoản và giao dịch của Customer. \\
\textbf{Precondition} & 
    Admin đã đăng nhập vào hệ thống quản trị;

    Hệ thống lưu trữ đầy đủ dữ liệu của Customer. \\
\textbf{Postcondition} & 
    Trạng thái sản phẩm/tài khoản/giao dịch được cập nhật đúng

    Customer nhận được thông báo từ hệ thống

    Log quản trị được lưu để truy vết sau này. \\
\hline
\vspace{0.025cm}
\textbf{Main flow} & 
\begin{enumerate}
    \item Admin chọn chức năng \textit{Moderation} trên hệ thống.
    \item Hệ thống hiển thị các tùy chọn:
    \begin{itemize}
        \item Product Moderation
        \item Account Moderation
        \item Monitor Transactions
    \end{itemize}
    \item Nếu Admin chọn \textit{Product Moderation}
    \begin{itemize}
        \item[3.1.] Hệ thống hiển thị danh sách sản phẩm chờ duyệt.
        \item[3.2.] Admin chọn một sản phẩm để \textit{Review Product}.
        \item[3.3.] Admin có thể:
        \begin{itemize}
            \item[3.3.1.] Approve / Reject sản phẩm $\rightarrow$ hệ thống cập nhật trạng thái sản phẩm.
            \item[3.3.2.] Delete Product nếu sản phẩm vi phạm nghiêm trọng.
        \end{itemize}
        \item[3.4.] Hệ thống lưu log lại quyết định của Admin và thông báo cho Customer liên quan.
    \end{itemize}
    \item Nếu Admin chọn \textit{Account Moderation}
    \begin{itemize}
        \item[4.1.] Hệ thống hiển thị danh sách tài khoản cần xử lý.
        \item[4.2.] Admin có thể:
        \begin{itemize}
            \item[4.2.1.] Ban Account (cấm tài khoản vi phạm).
            \item[4.2.2.] Unban Account (gỡ cấm tài khoản được xác minh lại).
        \end{itemize}
        \item[4.3.] Hệ thống lưu quyết định và thông báo cho Customer.
    \end{itemize}
    \item Nếu Admin chọn \textit{Monitor Transactions}
    \begin{itemize}
        \item[5.1.] Hệ thống hiển thị danh sách giao dịch đáng ngờ.
        \item[5.2.] Admin có thể:
        \begin{itemize}
            \item[5.2.1.] Apply Penalty (áp dụng hình phạt).
            \item[5.2.2.] Issue Warning (cảnh báo Customer).
        \end{itemize}
        \item[5.3.] Hệ thống lưu lại lịch sử xử lý và thông báo cho Customer.
    \end{itemize}
\end{enumerate} \\
\hline
\vspace{0.025cm}
\textbf{Alternative flow} &
\begin{itemize}
    \item[3.3.1a.] Nếu Admin từ chối duyệt sản phẩm $\rightarrow$ hệ thống yêu cầu nhập lý do từ chối.
    \item[4.2.1a.] Nếu Admin ban nhầm tài khoản $\rightarrow$ thực hiện \textit{Unban Account}.
    \item[5.1a.] Nếu hệ thống phát hiện gian lận giao dịch $\rightarrow$ tự động flag giao dịch cho Admin kiểm tra.
\end{itemize} \\
\hline
\end{tabular}
\caption{Use Case Scenario: Moderation}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../usecases/detailed/analytics.drawio.png}
    \caption{Sơ đồ Use Case cho Analysis}
    \label{fig:usecase}
\end{figure}
...
\hline
\vspace{0.025cm}
\textbf{Use Case} & \vspace{0.025cm} Analytics \\
\textbf{Actor} & Admin \\
\textbf{Mục tiêu} & Phân tích và giám sát dữ liệu về sản phẩm, hành vi người dùng, sự kiện và báo cáo tài chính để đưa ra quyết định quản lý. \\
\textbf{Precondition} & 
    Actor đã đăng nhập vào hệ thống

    Hệ thống có dữ liệu giao dịch, sự kiện, sản phẩm và hành vi người dùng được lưu trữ. \\
\textbf{Postcondition} & 
    Actor xem được số liệu trực quan về sản phẩm, người dùng, sự kiện và tài chính

    Báo cáo tài chính có thể lưu trữ hoặc gửi tự động

    Dữ liệu phân tích hỗ trợ Actor đưa ra quyết định quản lý. \\
\hline
\vspace{0.025cm}
\textbf{Main flow} & 
\begin{enumerate}
    \item Actor chọn chức năng \textit{Analytics} từ menu quản trị.
    \item Hệ thống hiển thị các tùy chọn:
    \begin{itemize}
        \item View Dashboard Overview
        \item Event Performance Analysis
        \item Finance Report
    \end{itemize}
    \item Nếu Admin chọn\textit{View Dashboard Overview}
    \begin{itemize}
        \item[3.1.] Hệ thống hiển thị tổng quan dữ liệu.
        \item[3.2.] Admin có thể xem chi tiết:
        \begin{itemize}
            \item[3.2.1.] Product Trends $\rightarrow$ thống kê xu hướng bán chạy, sản phẩm ít được quan tâm.
            \item[3.2.2.] User Behavior $\rightarrow$ phân tích cách người dùng truy cập, mua hàng, tìm kiếm.
        \end{itemize}
        \item[3.3.] Hệ thống cung cấp biểu đồ, có thể lọc theo thời gian.
    \end{itemize}
    \item Nếu Admin chọn \textit{Event Performance Analysis}
    \begin{itemize}
        \item[4.1.] Hệ thống hiển thị dữ liệu về hiệu suất sự kiện.
        \item[4.2.] Actor có thể phân tích số lượng tham gia, tỷ lệ chuyển đổi, doanh thu từ sự kiện.
        \item[4.3.] Hệ thống cho phép so sánh giữa nhiều sự kiện.
    \end{itemize}
    \item Nếu Admin chọn \textit{Finance Report}
    \begin{itemize}
        \item[5.1.] Hệ thống tạo báo cáo tài chính theo ngày/tháng/năm.
        \item[5.2.] Actor có thể xuất báo cáo hoặc lên lịch báo cáo tự động qua \textit{Automate Schedule}.
        \item[5.3.] Hệ thống lưu lại lịch sử báo cáo đã tạo.
    \end{itemize}
\end{enumerate} \\
\hline
\vspace{0.025cm}
\textbf{Alternative flow} &
\begin{itemize}
    \item[3.1a.] Nếu dữ liệu phân tích không đủ $\rightarrow$ hiển thị thông báo và gợi ý chọn phạm vi thời gian khác.
    \item[5.1a.] Nếu báo cáo tài chính bị lỗi $\rightarrow$ hệ thống thông báo ``Không thể tạo báo cáo, vui lòng thử lại''.
    \item[5.2a.] Nếu Admin đặt lịch trùng lặp trong \textit{Automate Schedule} $\rightarrow$ hệ thống yêu cầu xác nhận ghi đè.
\end{itemize} \\
\hline
\end{tabular}
\caption{Use Case Scenario: Analytics}
\end{table}
